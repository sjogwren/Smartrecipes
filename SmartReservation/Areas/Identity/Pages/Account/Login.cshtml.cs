...
namespace SmartReservation.Areas.Identity.Pages.Account
{
    ...
    public class LoginModel : PageModel
    {
            ...
            IConfiguration configuration, IHttpClientFactory factory, IExternalUserRole externalUserRole, UserManager<ExternalUser> userManager)
        {
            ...
            new ExternalUserService(configuration);
        }
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (result.Succeeded)
            {
                ...
                var roles = await _externalUserRole.GetRolesByUserId(user.ExternalUserID, "Recipe");
                _logger.LogInformation("User logged in.");
                foreach (var item in roles)
                {
                    if (item.Role == "Admin")
                    {
                        return Redirect(Url.Action("Currentrecipes", "Recipe"));
                    }
                    else
                    {
                        return Redirect(Url.Action("Currentrecipes", "Recipe"));
